...
int main()
	{
		...
		char *array_ptr_char[MAX_NUMBER_OF_BUCKETS<size_t>]{nullptr}, char_count_ascii {NORMAL_SPACE<char>};
		char chr_answer {}, chr_response {NULL_CHARACTER<char>};
		...
		size_t attemps {V_ZERO<size_t>}, counter = {V_ZERO<size_t>}, idx_array_ptr_char {};
				...
				while ((chr_response == V_CHAR_LOWER_Y<char> || chr_response == V_CHAR_UPPER_Y<char>) && (counter < MAX_NUMBER_OF_BUCKETS<size_t> && idx_array_ptr_char < MAX_NUMBER_OF_BUCKETS<size_t>))
					{
						...
						char_count_ascii = (char_count_ascii >= NORMAL_SPACE<char> && char_count_ascii <= ACCENT_MARK<char>) ? char_count_ascii : NORMAL_SPACE<char>;
						idx_array_ptr_char = (idx_array_ptr_char >= V_ZERO<size_t> && idx_array_ptr_char <= MAX_NUMBER_OF_BUCKETS<size_t>) ? idx_array_ptr_char : V_ZERO<size_t>;

						try
							{
								if ((array_ptr_char[idx_array_ptr_char++] = new char{char_count_ascii++}))	//Use the available memory of the 'Bucket' but do not save it.
									{
										/* Display of the addresses assigned to the flight. */
										std::cout << "Char Assignment #: [" << std::dec << counter++ + V_ONE<size_t> << "] of: [" << Bucket::number_of_buckets << "]." << std::endl;
										std::cout << "+ Item Index:\t\t<" << idx_array_ptr_char - V_ONE<size_t> << ">." << std::endl;
										std::cout << "+ Base Address:\t\t(" << &array_ptr_char << ")." << std::endl;
										std::cout << "+ Assigned Address:\t[" << static_cast<void*>(array_ptr_char[idx_array_ptr_char - V_ONE<size_t>]) << "]." << std::endl;
										std::cout << "+ Value Assigned:\t[" << static_cast<int>(*array_ptr_char[idx_array_ptr_char - V_ONE<size_t>]) << "] : {" << array_ptr_char[idx_array_ptr_char - V_ONE<size_t>] << "} = <" << *array_ptr_char[idx_array_ptr_char - V_ONE<size_t>] << ">." << std::endl << std::endl;
										...
										if ((chr_answer == V_CHAR_LOWER_Y<char>) || (chr_answer == V_CHAR_UPPER_Y<char>))
											{
												std::cout << "> Address released #:\t[" << idx_array_ptr_char - V_ONE<size_t> << "] = (" << static_cast<void*>(*(array_ptr_char + idx_array_ptr_char - V_ONE<size_t>)) << ") = {" << static_cast<int>(*(*(array_ptr_char + idx_array_ptr_char - V_ONE<size_t>)))<< "} = <" << *(*(array_ptr_char + idx_array_ptr_char - V_ONE<size_t>)) << ">." << std::endl << std::endl;

												counter--;
												char_count_ascii--;
												idx_array_ptr_char--;

												delete array_ptr_char[idx_array_ptr_char];
		...
		counter = V_ZERO<size_t>;
		std::cout << "Releasing the corresponding assigned pointers..." << std::endl;
		for (size_t idx{}; idx < MAX_NUMBER_OF_BUCKETS<size_t>; idx++)
			{
				if (array_ptr_char[idx])
					{
						std::cout << "> #: [" << counter++ << "]\t(" << static_cast<void*>(array_ptr_char[idx]) << ")\t[" << static_cast<int>(*array_ptr_char[idx]) << "]\t{" << *array_ptr_char[idx] << "}." << std::endl;
						delete *(array_ptr_char + idx);
						array_ptr_char[idx] = nullptr;
					}
			}

		/* Final results. */
		std::cout << "[" << counter << "] Freed memory addresses." << std::endl << std::endl;
		std::cout << "[" << attemps << "] Output results generated." << std::endl;
